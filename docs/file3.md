Perfect ✅ — below is a **ready-to-run PostgreSQL function** that matches the Knex migration pack you just received.

It’s production-safe and aligns exactly with your table structure (`offers`, `offer_eligibility_rules`, `topup_requests`, `transactions`, `users`, `user_activity`).

You can save this as `eligibility_function.sql` and run it via:

```bash
psql -d your_database -f eligibility_function.sql
```

or via Knex:

```js
await knex.raw(fs.readFileSync('./eligibility_function.sql', 'utf8'));
```

---

## 📄 `eligibility_function.sql`

```sql
-- =========================================================
-- Function: is_user_eligible_for_offer(offer_id, user_id)
-- Purpose : Evaluate eligibility rules dynamically for an offer
-- Author  : Mustapha Ibrahim project (Offers System)
-- =========================================================
...
BEGIN
  ...
  LOOP
    ...
    ELSIF r.rule_type = 'last_active_within' THEN
      ...
      SELECT (u.last_seen >= now() - ((r.params ->> 'days')::int * interval '1 day'))
      INTO pass
      FROM users u WHERE u.id = p_user;
...
COMMENT ON FUNCTION is_user_eligible_for_offer IS
  'Evaluates all offer_eligibility_rules for a given offer and user; returns true if eligible based on AND/OR logic.';
```
...
During your offer redemption (inside your Express + Knex transaction):
...
if (!rows[0].eligible) {
  throw new Error('User not eligible for this offer');
}
```
...
```sql
SELECT is_user_eligible_for_offer('<offer_uuid>', '<user_uuid>');
```

If it returns `true`, the user can redeem; otherwise `false`.
